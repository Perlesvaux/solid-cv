...
export default function Fieldeducation({ getter, setter }){
  ...
  return (
    <>
      ...
      <label>Image
        <input type="text" name="image" value={newEntry.image} onChange={handleNewEntry} />
      </label>
...

      {
        getter.education.map(({institution, title, url, image}, indx) => 
          <div key={indx}>{institution} {title} {url} {image} <button onClick={()=>deleter(indx)}>x</button></div>)
      }
    </>
  )
}
